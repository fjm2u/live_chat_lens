...
![Live Chat Lensで配信中に拾いたいコメントだけを見る](https://raw.githubusercontent.com/fjm2u/live_chat_lens/main/docs/images/live-chat-lens-streamer-overview.jpg)

![Live Chat Lensがコメントを絞り込み、面白い順に表示する流れ](https://raw.githubusercontent.com/fjm2u/live_chat_lens/main/docs/images/live-chat-lens-comment-ranking-flow.jpg)

![Live Chat LensをOBS内で使う手順](https://raw.githubusercontent.com/fjm2u/live_chat_lens/main/docs/images/live-chat-lens-obs-setup.jpg)

## 起動

1. PluginをインストールしてOBSを起動します。`Docks → AI Comments` を表示します。
2. YouTube Live URLまたはVideo IDとJev APIキーを入力します。YouTube認証は既存ファイルを自動設定します。
3. YouTube認証の検出順は `YOUTUBE_API_KEY` → `YOUTUBE_OAUTH_FILE` → 前回選択したファイル → `~/.config/english-video-growth/youtube-token.json`。指定済みのファイルが消えた場合は別アカウントへ自動切り替えせず、手動設定を表示します。`変更` から別の認証ファイルやAPIキーを指定できます。記憶するのはファイルの場所のみです。リフレッシュトークンはメモリ内で使用し、Googleの固定token endpointで更新します。新規ログイン・権限追加・元ファイル更新は行いません。
4. `接続`。配信中のチャットが有効な動画が必要です。初回ページは履歴として読み飛ばし、その後の新着を推薦します。
5. `★` はhigh、上位4件を強調、最大10件。👍は良い推薦、×は候補から削除します。

JevキーとYouTubeキーは別です。公式YouTube Data APIの認証には、既存OAuth認証またはGoogle側のAPIキーを使用します。OBSの配信ログインを直接共有する実装ではありません。公開動画の読み取り用途で、キーのAPI制限をYouTube Data API v3に設定してください。アカウントへの投稿やBAN操作はありません。

macOSではJevキーをキーチェーンに保存して次回起動時に復元します。YouTube APIキーはメモリのみで保持します。起動環境の `TYPESAFE_API_KEY` / `YOUTUBE_API_KEY` も利用できます。キーを設定INI・ログ・コマンド履歴・Gitに保存しないでください。
...
macOSの `package-macos.sh` はQtリンク先をOBS同梱Frameworkへ変更し、ローカルのad-hoc署名を付けます。Homebrew Qtの二重ロードを防ぎます。OBSに不足しているQt SecureTransport TLS backendをPlugin内に同梱します（証明書検証は有効のまま）。再ビルド後は必ず再実行してください。配布用のDeveloper ID署名・公証は未実施です。Windows/LinuxはSDK向けCMake設定を用意していますが実機検証はしていません。
...
5. `Engine`がイベントを`events.jsonl`に記録します。設定INIにはVideo URL・表示条件・OAuthファイルの場所を保存し、APIキーとコメント本文は保存しません。JevキーはmacOSキーチェーンで保持します。
...
設定内の「AIの判定方針を編集」から、非表示条件や優先する話題を日本語で編集できます。1〜6000文字。保存した方針は次のAPI評価から反映し、再起動後も復元します。「初期値に戻す」は編集欄を戻す操作で、保存を押すまで確定しません。送信済み・表示済みのコメントは再評価しません。選択肢の形式とコメントを命令として扱わない制約は固定です。方針は設定INIに保存されJevへ送信されるため、APIキーなどの秘密は入力しないでください。


### Jevキーの保存（macOS）

入力完了時と接続時にmacOSキーチェーン（service: `obs-comment-dock`、account: `jev-api-key`）へ保存し、次回起動時に自動入力します。キーチェーン操作はworkerで実行し、失敗時はDockに通知します。設定INI・ログには保存しません。YouTube OAuthは元ファイルを使い、場所のみ記憶します。その他OSの安全な永続化は未対応です。
