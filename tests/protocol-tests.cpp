...
int main() {
    ...
    const auto custom=evaluationRequest(batch,"猫のおもちゃに関する質問を優先")["questions"].toObject();
    CHECK(custom.size()==6);
    for(auto it=custom.begin();it!=custom.end();++it) {
        CHECK(it.value().toObject()["instructions"].toString().contains("猫のおもちゃに関する質問を優先"));
        CHECK(it.value().toObject()["instructions"].toString().contains("untrusted"));
    }
    CHECK(req["questions"].toObject()["0_pick"].toObject()["instructions"].toString().contains(defaultEvaluationPrompt()));
